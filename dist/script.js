...
function () {
  ...
  _createClass(Customizator, [{
    ...
    value: function onScaleChange(e) {
      ...
      var recursy = function recursy(element) {
        element.childNodes.forEach(function (node) {
          //првоеряем если ли в тегах текст и вообще какое-либо значние, в том числе и пробелы
          if (node.nodeName === '#text' && node.nodeValue.replace(/\s+/g, "").length > 0) {
            if (!node.parentNode.getAttribute('data-fz')) {
              var value = window.getComputedStyle(node.parentNode, null).fontSize; //получаем размер шрифта каждого элемента
    ...
    value: function onColorChange(e) {
      var body = document.querySelector('body');
      body.style.backgroundColor = e.target.value; // console.log('e.target.value: ', e.target.value);
